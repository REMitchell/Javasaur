...
svgEditor.addExtension("animate", function(S) {
		...
		function getStyle(opts) {
			// if we are in animate mode, we don't want to disable the eye-dropper tool
			var mode = svgCanvas.getMode();
			if (mode == "animate") return;

			var elem = null;
			var tool = $('#tool_animate');
...
*/
		return {
			name: "animate",
			svgicons: "extensions/animate-icon.xml",
			buttons: [{
				id: "tool_animate",
				type: "mode",
				...
				events: {
					"click": function() {
						//svgCanvas.setMode("animate");
                        //openWindow("animation.php");
                        var animateText = "asdf asdf as s dfsdafdsaf";
                        TINY.box.show({iframe:'http://javasaur.com/animation.php?pid=2&elem=5',boxid:'frameless',width:750,height:450,fixed:false,maskid:'bluemask',maskopacity:40,closejs:function(){closeJS()}});
